...
# -----------------------------------------------------------------------------------------
# Same parameters as the CRNN GAN
...
flags.DEFINE_integer("tones_per_cell", 2,             # 2,3
...
class RNNGAN(object):
  ...
  def __init__(self, is_training, num_song_features=None, num_meta_features=None):
    ...
    self._input_metadata_fake = tf.placeholder(shape=[None, num_meta_features], dtype=data_type(),name="input_metadata_fake")
    ...
    with tf.variable_scope('G') as scope:
      ...
      # -----------------------------------------------------------------------------------------
      # Removed the generate meta data
      # -----------------------------------------------------------------------------------------
    ...
    with tf.variable_scope('D') as scope:
      ...
      # -----------------------------------------------------------------------------------------
      # Real and Fake data for conditional Gan
      ...
      songdata_fake_condition_inputs = [tf.concat([self._input_metadata_fake, songdata_input],1) for songdata_input in songdata_inputs]
      self.fake_d,self.fake_d_features = self.discriminator(songdata_fake_condition_inputs, is_training, msg='wrong')
    ...
    # -----------------------------------------------------------------------------------------
    # Added a fake_d to the discriminator loss
    self.d_loss = tf.reduce_mean(-2*tf.log(tf.clip_by_value(self.real_d, 1e-1000000, 1.0)) \
                                 ...
                                 -2*tf.log(1 - tf.clip_by_value(self.fake_d, 0.0, 1.0-1e-1000000)))
                                 # Chose 2.3 because I wanted to penalize the fake more
    # -----------------------------------------------------------------------------------------
  ...
  # -----------------------------------------------------------------------------------------
  @property
  def input_metadata_fake(self):
    return self._input_metadata_fake
  # -----------------------------------------------------------------------------------------
...
def run_epoch(session, model, loader, datasetlabel, eval_op_g, eval_op_d, pretraining=False, verbose=False, run_metadata=None, pretraining_d=False):
  ...
  g_loss, d_loss = 15.0, 15.0 # Was 10, changed to 15 each
...
def main(_):
  ...
  restore_flags()
  wantedgenre = sys.argv[12]
  print(wantedgenre) #jazz, classical, both 
  ...
  with tf.Graph().as_default(), tf.Session(config=tf.ConfigProto(log_device_placement=FLAGS.log_device_placement)) as session:
    ...
    # From old CRNN-GAN, We resave the last data point
